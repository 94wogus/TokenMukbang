...
- **Click → focus terminal** — best-effort: matches the session's controlling TTY to a
  ...
  activated; failures are silent. Sessions running in a **VS Code / Cursor integrated
  terminal** bring that editor forward (the whole app — there's no API for the exact panel).
- **Session-finished alerts** — when any Claude Code session ends its turn (it's done and
  waiting on you), a notification fires; **tap it to jump to that session's terminal**.
  Detected live from the transcript (no polling lag), toggleable in Settings → Alerts
  ([ADR-0022](docs/adr/0022-session-completion-notification.md)).
...
- **Value / savings (Now tab)** — what this billing period's tokens would cost at API
  list rates vs your flat subscription, so you see how much your plan is actually worth
  (e.g. `$16,619 at API rates · 75× value`). Set your plan price + billing day in
  Settings → General. Local-only estimate — [ADR-0021](docs/adr/0021-value-savings-estimate.md).
