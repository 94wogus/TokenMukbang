...
| level | 김 빛깔(steam) | 밀도 / 높이 | 응결 | 언더글로우(glow.broth) |
|---|---|---|---|---|
| **calm** | `#FFFFFF` @ 8% | 가는 한 줄기, 짧게 | 거의 없음 | 미지근한 웜앰버 `#F0A93C` @ 12% |
| **watch** | `#FFE3B8` @ 16% | 두 줄기, 중간 높이 | 모서리 1~2방울 | 허니 `#F2A226` @ 22% |
| **warning** | `#FFB877` @ 24% | 넓은 plume, 상단 절반 | 상단 유리 응결 번짐 | 파프리카 `#F07A22` @ 34% |
| **critical** | `#F08A5A`→`#E84A2C` @ 36% | 상단 가득, 두꺼운 plume | 상단 전체 응결+흘러내림 | 엠버레드 `#E84A2C` @ 46%(낮게 맥동) |

> light 모드 김 빛깔은 동일 hue를 명도만 낮춰 적용(흰 종이 위에서 김이 떠 보이게): calm `#9AA0A8`@10%,
> watch `#C8923D`@18%, warning `#C2611E`@24%, critical `#B5391F`@32%. **메뉴바 색은 별도 정의(§4.1)** —
> 반투명 메뉴바에서 김이 거의 안 보이므로 메뉴바는 김보다 *김-틴트된 숫자 색*에 의존.
