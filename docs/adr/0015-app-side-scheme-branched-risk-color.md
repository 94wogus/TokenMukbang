...
초기 구현에서 `TokenMukbangKit` 은 위험도(risk)를 색으로 환산해 `UsageSnapshot.Window.riskHex`
...
옮기고, scheme 별로 분기한다. `TokenMukbangKit` 은 색을 만들지 않고 의미 레벨만 내보낸다.**
...
- `RiskTone.color(level:over:scheme:)` 가 `(레벨, over, 라이트/다크)` → `Color` 를 해석하는
  앱-측 지점이다. 컨텍스트 채움(세션)도 같은 팔레트 문법을 재사용한다 — 두 번째 위험 *의미* 팔레트를
  만들지 않는다.
- **(2026-06-13 추가, theme-palette-redesign)** **팝오버 UI 의 위험색 리졸버는 per-theme
  `ThemeMood.risk(level:over:)` 다.** `ThemeMood.resolve` 가 선택된 테마의 curated 4단계(+over)
  위험 램프를 굽고, 팝오버 뷰들은 `@Environment(\.themeMood)` 로 읽는다(`GaugeBar` 는 `mood.riskRamp`).
  `RiskTone.color` 는 **메뉴바**(theme-independent, wallpaper-tuned)·**위젯**에서 유지 — 테마는 팝오버
  *분위기* 에만 적용되고 메뉴바/위젯은 테마 중립이라서. 둘 다 앱 측이며 Kit 은 색-free(ADR-0001),
  위험 의미(calm→critical 순서·빨강=위험)도 양쪽 불변. 즉 리졸버는 표면별로 둘(`mood.risk`=팝오버,
  `RiskTone.color`=메뉴바·위젯)이되 **두 번째 의미 체계가 아니라 같은 문법을 테마별로 틴트**한 것.
- 모든 UI 표면(팝오버·메뉴바·위젯·오버레이)은 `@Environment(\.colorScheme)` 를 읽어 리졸버에 넘긴다.
  게이지는 공용 `GaugeBar` 컴포넌트가 그린다(테마 램프는 `mood`, 메뉴바/위젯은 `RiskTone`).
...
- `Sources/TokenMukbangKit/UsageSnapshot.swift` (`Window.riskLevel`/`isOver`), `UsageService.swift`
- `App/TokenMukbang/` 의 모든 뷰 + `TokenMukbang.swift`(`MenuBarLabel`)
- `App/TokenMukbangWidget/UsageWidget.swift`
