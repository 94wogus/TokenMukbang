...
1. **`ProcessRunning` seam 경유 (ADR-0006).** 새 `RetrospectiveSummarizing` 프로토콜 +
   ...
   **drain(=output+fresh-input+cache-write)**·프롬프트수·프롬프트당 토큰·모델·시간분포) +
   소량의 균형 잡힌 프롬프트 샘플이다 — turn 수 많은 자동 세션(ralph 등)에 편향되지 않게 하기
   위함(코치는 "토큰은 큰데 프롬프트 적은" 자동/롱컨텍스트 패턴을 오히려 짚어준다). **plan-aware**:
   지표에 플랜(예: Max)을 함께 넣어, 구독 플랜이면 비용 조언을 *$* 가 아니라 **5h/7d 사용량
   윈도우 소진**으로 프레이밍한다(이 앱이 추적하는 바로 그 한도와 정합). **무엇이 그 윈도우를
   태우나는 사실관계가 load-bearing**이다: 순위는 (1) 모델 티어(Opus가 제일 빠르게 태움) →
   (2) output → (3) uncached input → (4) cache-write 이고, **cache *read* 는 한도에 거의
   카운트되지 않는(≈0.1×, ITPM 미카운트) near-free 재사용**이라 drain에서 제외한다. 따라서
   프롬프트·지표는 cache-read(또는 cache-read/turn)를 "비용·고칠 문제"로 프레이밍하지 않으며
   (그건 컨텍스트 크기 proxy일 뿐), 코치가 데이터에 없는 숫자(임의 multiplier)·지표 테이블
   밖 프로젝트·**데이터에 없는 인과/메커니즘**(예: ">5분 idle gap 때문" 같은 턴-간격 단정 —
   지표에 타이밍 정보가 없다)을 단정하지 못하도록 hard-rule 가드레일을 둔다. 또한 **코치·Copy
   리포트·인앱 Menu의 프로젝트 표시 상한은 단일 상수(`RetrospectiveMetrics.maxListedProjects`)로
   통일**해, 코치가 사용자가 메뉴에서 못 보는 프로젝트를 인용하는 일을 막는다. (초기 구현은
   cache-read/turn을 전면에 내세워 "캐시 리드가 한도를 갉아먹는다"는 거꾸로 된 프레이밍을 유발하고,
   코치 top-10 vs 리포트 top-8 컷 불일치로 메뉴 밖 프로젝트를 인용 — 2026-06-23 교정.)
