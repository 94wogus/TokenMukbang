...
1. **`ProcessRunning` seam 경유 (ADR-0006).** 새 `RetrospectiveSummarizing` 프로토콜 +
   ...
   **drain(=output+fresh-input+cache-write)**·프롬프트수·프롬프트당 토큰·모델·시간분포) +
   ...
   (그건 컨텍스트 크기 proxy일 뿐), 코치가 데이터에 없는 숫자(임의 multiplier)나 지표 테이블
   밖 프로젝트를 인용하지 못하도록 hard-rule 가드레일을 둔다. (초기 구현은 cache-read/turn을
   전면에 내세워 "캐시 리드가 한도를 갉아먹는다"는 거꾸로 된 프레이밍을 유발 — 2026-06-23 교정.)
