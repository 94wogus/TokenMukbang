...
| [0023](0023-local-otlp-telemetry-receiver.md) | Claude Code 텔레메트리는 로컬 루프백 OTLP receiver로 수신 | Accepted | 2026-06-30 | Claude Code의 내장 OTEL을 `127.0.0.1` OTLP/HTTP(JSON, :4318) receiver로 직접 ingest — 트랜스크립트 휴리스틱보다 정확·풍부(수락률·LoC·커밋/PR·active time). **첫 inbound 경계**: 루프백 전용·opt-in(기본 off)·앱 전용 저장(위젯 스냅샷 불변, ADR-0003). 콘텐츠 필드는 디코더가 미매핑(서버 강제 redaction). 와이어 파싱은 순수 Kit, 소켓은 App. 신규 egress 없음 |
