...
| [0024](0024-app-managed-telemetry-and-consented-forwarding.md) | 앱이 텔레메트리를 설정하고 동의 하에 회사로 forward | Accepted | 2026-06-30 | ADR-0023 정밀화. **Slice 1(#29):** 앱이 `~/.claude/settings.json`을 안전 머지해 Claude Code를 로컬 receiver로 자동 배선("설치=설정완료", egress 0). 파싱 불가 파일은 미수정·안내(클로버 금지). **Slice 2:** ingest 샘플을 `OTLPEncoder`로 재인코딩(콘텐츠 제거 후)해 회사 OTLP로 forward — **명시적 동의 + Settings 수동 enrollment(엔드포인트+토큰) 하에서만**, 기본 off. 사내 전용 positioning. "egress 없음"→"무동의 egress 없음" |
