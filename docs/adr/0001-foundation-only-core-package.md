# ADR-0001: 모든 로직은 Foundation-only 패키지 `ClaudeUsageKit`에 둔다
...
`ClaudeUsageKit`에 두고, 이 패키지는 오직 `Foundation`만 import** 한다. AppKit·SwiftUI·
...
- 제약: `App/`에서 로직을 작성하고 있다면 거의 항상 `ClaudeUsageKit`로 내려야 한다.
...
- `Sources/ClaudeUsageKit/**`, `Package.swift`, `App/Shared/`
